...
app.get("/api/recommendations/:bookId", async (req, res) => {
  try {
    ...
    console.log(
      "Processing recommendation request for book:",
      bookId,
      "userId:",
      userId
    );
    ...
    if (userId) {
      try {
        ...
        if (user) {
          ...
          // Check if finishedBooks array exists and contains at least 3 books with ratings
          // Lowered threshold to 3 for better compatibility with the new implementation
          if (user.finishedBooks && Array.isArray(user.finishedBooks)) {
            // Count books that have an actual rating (not null)
            const ratedBooksCount = user.finishedBooks.filter(
              (book) => book.rating !== null && book.rating !== undefined
            ).length;

            console.log(
              `User has ${ratedBooksCount} rated books (need 3+ for collaborative filtering)`
            );
    ...
    console.log(
      `Collaborative filtering: ${
        useCollaborativeFiltering ? "Yes" : "No"
      }, User ID: ${userId || "None"}`
    );
    ...
    python.on("close", async (code) => {
      ...
      try {
        ...
        // Ensure we're parsing valid JSON by trimming any extra output
        let jsonData = dataString.trim();
        // Find the first '[' character, which should be the start of JSON array
        const startIndex = jsonData.indexOf('[');
        if (startIndex > 0) {
          jsonData = jsonData.substring(startIndex);
        }
        
        const recommendations = JSON.parse(jsonData);
        console.log(
          `Successfully parsed recommendations: ${recommendations.length} items found`
        );
        ...
        if (user) {
          ...
          console.log(
            `Extracted ${bookIds.length} book IDs from recommendations`
          );

          // Update user's recommendations
          if (bookIds.length > 0) {
            console.log(
              `User's current recommendations count: ${
            ...
            try {
              ...
              await User.findByIdAndUpdate(
                user._id,
                { recommendations: bookIds }, // Replace previous recommendations with new ones
                { new: true }
              );
              console.log(
                `Successfully saved user's recommendations to database (replacing previous recommendations)`
              );
            } catch (saveError) {
            ...
            console.log(
              "No valid book IDs found in recommendations, skipping update"
            );
          }
        } else {
...
app.get("/api/recommendations_multiple", async (req, res) => {
  try {
    ...
    console.log(
      `Processing recommendation request for books, userId: ${userId}`
    );
    ...
    for (const bookId of bookIdArray) {
      ...
      if (!book) {
        ...
        return res
          .status(404)
          .json({ error: `Book not found in database: ${bookId}` });
      }
    }
    ...
    if (useCollaborativeFiltering) {
      ...
      args.push(...bookIdArray.filter(id => id)); // Filter out empty IDs
    }
...
app.post("/api/user/currently-reading", auth, async (req, res) => {
  try {
    ...
    console.log(
      `Before update - Currently reading: ${user.currentlyReading.length}, Finished: ${user.finishedBooks.length}`
    );
    ...
    if (!action || action === "add") {
      ...
      console.log(
        `Book is${isCurrentlyReading ? "" : " not"} in currently reading list`
      );
      ...
      const existingIndex = user.finishedBooks.findIndex(
    ...
    console.log(
      `After update - Currently reading: ${user.currentlyReading.length}, Finished: ${user.finishedBooks.length}`
    );
...
app.post("/api/user/rate-book", auth, async (req, res) => {
  try {
    ...
    const bookIndex = user.finishedBooks.findIndex(
      (item) => item.bookId === bookId
    );
